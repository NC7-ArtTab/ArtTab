...
CREATE TABLE art (
                       ...
                       art_category VARCHAR(1) NOT NULL COMMENT '작품구분 => 동양화 : E, 서양화:W, 공예품:P, 기타:X',
                       art_writer VARCHAR(20) NULL DEFAULT '작자미상' COMMENT '작가',
                       art_content TEXT NULL COMMENT '작품설명',
...
CREATE TABLE attach (
                          ...
                          file_status VARCHAR(1) NULL DEFAULT 'Y' COMMENT '파일사용여부 => 사용중(게시건), N:미사용(삭제건)',
                          PRIMARY KEY (file_no),
                          FOREIGN KEY (art_no) REFERENCES art (art_no)
);
...
CREATE TABLE pay (
                       ...
                       pay_status VARCHAR(1) NULL DEFAULT 'R' COMMENT '결제상태 =>  R:입금대기중 Y:결제완료',
